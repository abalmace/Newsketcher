YUI().add('objetivesview',function (Y)
{

	var Lang = Y.Lang;

	/* MyComponent class constructor */
	function ObjetivesView()
	{
		ObjetivesView.superclass.constructor.apply(this, arguments);
	}
	ObjetivesView.NAME = "objetivesView";
	...
	Y.extend(ObjetivesView, Y.Base,
	{
		...
		{	
			...
			events: {
				...
				'#new-todo-objetives': {keypress: 'createTodo'},
			...
			initializer: function () {
				...
				self.objetiveList = this.todoList = new TodoList();
				var list = self.objetiveList;
			...
			render: function () {
				...
				stats    = this.container.one('#todo-stats-objetives'),
			...
			add: function (e) {
				...
				this.container.one('#todo-list-objetives').append(view.render().container);
			},
			...
			reset: function (e) {
				...
				this.container.one('#todo-list-objetives').setContent(fragment);
			}
			});
			...
			enter: function (e) {
				if (e.keyCode === 13) { // enter key
				Y.one('#new-todo-objetives').focus();
				}
			},
		...
		{
			var list = this.objetiveList.remaining();
			var objetives = [];
			Y.each(list, function(element)
			{
				objetives.push({description:element.getAsHTML('text')});
			});
			
			return objetives;
		}
	});
	Y.namespace("ModuleList").ObjetivesView= ObjetivesView;

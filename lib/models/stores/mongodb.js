...
var INSTANCESUBTASK_REGEXP = /^\/channel\/([a-zA-Z0-9_-]+)\/instanceSubTasks$/;
...
*/
function getInstanceSubTasks(subtaskGUID,userGUID, callback)
{
	db.collection('InstanceSubTask', function(err, collection)
	{
		collection.find({subTaskGuid:subtaskGUID , group:{$elemMatch :{ guid : userGUID, selected:true }}},function(err, cursor)
		{
			cursor.toArray(function(err, results)
			{

...
var INSTANCESUBTASK_REGEXP = /^\/channel\/([a-zA-Z0-9_-]+)\/([a-zA-Z0-9_-]+)\/instanceSubTasks$/;
...
/*
	Function que retorna las instanceSubTasks.
*/
function getInstanceSubTasks(circleGUID,subtaskGUID,userGUID, callback)
{
	db.collection('InstanceSubTask', function(err, collection)
	{
		collection.find({circleGuid:circleGUID, subTaskGuid:subtaskGUID , group:{$elemMatch :{ guid : userGUID, selected:true }}},function(err, cursor)
		{
			cursor.toArray(function(err, results)
			{
				callback({ instanceSubTasks: results || []});
			});
		});
	});
}
...
/*
	Function que retorna todas las instanceSubTasks de una subtask.
*/
function getAllInstanceSubTasks(circleGUID,subtaskGUID, callback)
{
	db.collection('InstanceSubTask', function(err, collection)
	{
		collection.find({circleGuid:circleGUID, subTaskGuid:subtaskGUID},function(err, cursor)
		{
			cursor.toArray(function(err, results)
			{
				callback({ instanceSubTasks: results || []});
...
var listener = {
	incoming: function(message, callback)
	{
		...
		else if(match = INSTANCESUBTASK_REGEXP.exec(message.channel))
		{
			...
			switch(message.data.status)
			{
				...
				case 'delete2':
					...
					db.collection('InstanceSubTask', function(err, collection)
					{
						sys.log('probando '+message.data.guid);
...
module.exports =
{
  	...
  	,getInstanceSubTasks: getInstanceSubTasks
  	,getAllInstanceSubTasks:getAllInstanceSubTasks
  	,getTasks: getTasks

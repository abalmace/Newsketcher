...
MapSketcher.prototype.init = function() {
  ...
  self.app.post('/config.json', function(req, res){
    res.writeHead(200, {
  ...
  self.app.post('/rooms/:room/sketches.json', function(req,res) {
    res.writeHead(200, {
      'Content-Type': 'application/json'
    });
  ...
  self.app.post('/room/:taskCollection/tasks.json', function(req,res) {
    res.writeHead(200, {
      'Content-Type': 'application/json'
    });
...
self.app.post('/channel/:taskCollection/:taskGuid/task.json', function(req,res)
{
...
self.app.post('/channel/:circleGUID/:subtaskGUID/:userGUID/instanceSubTasks.json', function(req,res)
{
    res.writeHead(200, {
      'Content-Type': 'application/json'
    });
	store.getInstanceSubTasks(req.params.circleGUID,req.params.subtaskGUID,req.params.userGUID, function(taskName)
	{
	...
	self.app.post('/channel/:circleGUID/:subtaskGUID/instanceSubTasks.json', function(req,res)
	{
		res.writeHead(200, {'Content-Type': 'application/json'});
		store.getAllInstanceSubTasks(req.params.circleGUID,req.params.subtaskGUID, function(taskName)
		{
			res.write(JSON.stringify(taskName));
			res.end();
		});
    ...
    self.app.post('/rooms/:task/roommates.json', function(req,res) {
    res.writeHead(200, {
      'Content-Type': 'application/json'
    });
	...
	self.app.post('/channel/:userType/people.json', function(req,res)
	{
		res.writeHead(200,{'Content-Type': 'application/json'});
	...
	*/
	self.app.post('/channel/:dataBase/:userGuid/circles.json', function(req,res)
	{
		res.writeHead(200,{'Content-Type': 'application/json'});
	...
	self.app.post('/channel/:dataBase/circles.json', function(req,res)
	{
		res.writeHead(200,{'Content-Type': 'application/json'});
	...
	self.app.post('/channel/:dataBase/:userName/:userPass/person.json', function(req,res)
	{
		res.writeHead(200,{'Content-Type': 'application/json'});
	...
	//Guarda al usuario
	self.app.post('/channel/:dataBase/:userName/:userNick/:userPass/:userType/:guid/bool.json', function(req,res)
	{
		res.writeHead(200,{'Content-Type': 'application/json'});
...
self.bayeux.bind('publish', function(clientId,channel,data)
{
	if(channel == '/user/connect')
		self.clientsInformation[clientId] = {name:data.name, guid:data.guid}
}) 

self.bayeux.bind('disconnect',function(clientId)
{
	var client = self.clientsInformation[clientId];
	if(client)
	{
		self.bayeux.getClient().publish('/user/disconnect',client);
		delete self.clientsInformation[clientId];
	}
});
